...
function playBeep() {
  // Create a new audio context. Some browsers require AudioContext to be created on user interaction
  const AudioContext = window.AudioContext || window.webkitAudioContext;
  const audioCtx = new AudioContext();
  const oscillator = audioCtx.createOscillator();
  const gainNode = audioCtx.createGain();
  oscillator.type = 'sine';
  oscillator.frequency.setValueAtTime(880, audioCtx.currentTime); // 880 Hz tone (A5)
  // Set volume
...
if (testSelectedButton) {
  testSelectedButton.addEventListener('click', () => {
    ...
    if (bed) {
      // Set its level to a very low value (2%) to simulate near-empty state
      // regardless of its current level
      bed.level = 2;
      renderBeds();
      // Remove from alerted set so that alert will fire if it reaches 0
      alertedBeds.delete(bed);
      showAlert(
        t('testAlert')
          .replace('{floor}', floorValue)
          .replace('{bed}', bedId)
      );
    }
  });
